...
import Base: ==
import ...Pkg25: Pkg25Error, isdevmetadata
using ..Types
...
immutable Requirement <: Line
    ...
    function Requirement(content::AbstractString)
        ...
        isempty(fields) && throw(Pkg25Error("invalid requires entry: $content"))
        package = shift!(fields)
        all(field->ismatch(Base.VERSION_REGEX, field), fields) ||
            throw(Pkg25Error("invalid requires entry for $package: $content"))
        versions = VersionNumber[fields...]
        issorted(versions) || throw(Pkg25Error("invalid requires entry for $package: $content"))
...
function dependents(packagename::AbstractString)
    ...
    cd(Pkg25.dir()) do
        for (pkg,latest) in Pkg25.Read.latest(devmetadata)
