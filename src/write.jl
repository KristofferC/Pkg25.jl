...
import Base.LibGit2, ..Cache, ..Read, ...Pkg25.Pkg25Error, ..isdevmetadata
...
function prefetch(pkg::AbstractString, sha1::AbstractString)
    ...
    throw(Pkg25Error("$pkg: couldn't find commit $(sha1[1:10])"))
end
...
function fetch(repo::GitRepo, pkg::AbstractString, sha1::AbstractString)
    ...
    if isempty(url)
        throw(Pkg25Error("$pkg: $f failed to get commit $(sha1[1:10]), please file a bug report with the package author."))
    else
        throw(Pkg25Error("$pkg: $f failed to get commit $(sha1[1:10]), please file an issue at $url"))
    end
end
