...
import ..Pkg25: DEFAULT_META, META_BRANCH, Pkg25Error
...
function cd(f::Function, args...; kws...)
    ...
    if !isdir(metadata_dir)
        !haskey(ENV,"JULIA_PKGDIR") ? init() :
            throw(Pkg25Error("Package metadata directory $metadata_dir doesn't exist; run Pkg25.init() to initialize it."))
    end
