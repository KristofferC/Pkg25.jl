...
type Pkg25Error <: Exception
    msg::AbstractString
    ex::Nullable{Exception}
end
Pkg25Error(msg::AbstractString) = Pkg25Error(msg, Nullable{Exception}())
function Base.showerror(io::IO, pkgerr::Pkg25Error)
    print(io, pkgerr.msg)
    if !isnull(pkgerr.ex)
        pkgex = get(pkgerr.ex)
...
Equivalent to `normpath(Pkg25.dir(),names...)` – i.e. it appends path components to the
package directory and normalizes the resulting path. In particular, `Pkg25.dir(pkg)` returns
...
Initialize `Pkg25.dir()` as a package directory. This will be done automatically when the
`JULIA_PKGDIR` is not set and `Pkg25.dir()` uses its default value. As part of this process,
...
Opens `Pkg25.dir("REQUIRE")` in the editor specified by the `VISUAL` or `EDITOR` environment
variables; when the editor command returns, it runs `Pkg25.resolve()` to determine and install
...
Remove all requirement entries for `pkg` from `Pkg25.dir("REQUIRE")` and call `Pkg25.resolve()`.
"""
...
Add a requirement entry for `pkg` to `Pkg25.dir("REQUIRE")` and call `Pkg25.resolve()`. If
...
If `pkg` has a URL registered in `Pkg25.dir("METADATA")`, clone it from that URL on the
...
in `Pkg25.dir("METADATA")`. The package repo is cloned by the name `pkg` if provided; if not
...
Checkout the `Pkg25.dir(pkg)` repo to the branch `branch`. Defaults to checking out the
"master" branch. To go back to using the newest compatible released version, use
`Pkg25.free(pkg)`. Changes are merged (fast-forward only) if the keyword argument `merge ==
...
Free the package `pkg` to be managed by the package manager again. It calls `Pkg25.resolve()`
to determine optimal package versions after. This is an inverse for both `Pkg25.checkout` and
`Pkg25.pin`.

You can also supply an iterable collection of package names, e.g., `Pkg25.free(("Pkg251",
"Pkg252"))` to free multiple packages at once.
"""
...
Update the metadata repo – kept in `Pkg25.dir("METADATA")` – then update any fixed packages
that can safely be pulled from their origin; then call `Pkg25.resolve()` to determine a new
...
optimal set of package versions is based on the contents of `Pkg25.dir("REQUIRE")` and the
state of installed packages in `Pkg25.dir()`, Packages that are no longer required are moved
into `Pkg25.dir(".trash")`.
"""
...
dependencies in depth-first recursive order. This is called automatically by `Pkg25.resolve()`
...
# point users to Pkg25Dev
register(args...) =
    error("Pkg25.register(pkg,[url]) has been moved to the package Pkg25Dev.jl.\n",
          "Run Pkg25.add(\"Pkg25Dev\") to install Pkg25Dev on Julia v0.5-")

tag(pkg, ver=nothing, commit=nothing) =
    error("Pkg25.tag(pkg, [ver, [commit]]) has been moved to the package Pkg25Dev.jl.\n",
          "Run Pkg25.add(\"Pkg25Dev\") to install Pkg25Dev on Julia v0.5-")

publish() =
    error("Pkg25.publish() has been moved to the package Pkg25Dev.jl.\n",
          "Run Pkg25.add(\"Pkg25Dev\") to install Pkg25Dev on Julia v0.5-")

generate(pkg, license) =
    error("Pkg25.generate(pkg, license) has been moved to the package Pkg25Dev.jl.\n",
          "Run Pkg25.add(\"Pkg25Dev\") to install Pkg25Dev on Julia v0.5-")

license(lic=nothing) =
    error("Pkg25.license([lic]) has been moved to the package Pkg25Dev.jl.\n",
          "Run Pkg25.add(\"Pkg25Dev\") to install Pkg25Dev on Julia v0.5-")

submit(pkg, commit=nothing) =
    error("Pkg25.submit(pkg[, commit]) has been moved to the package Pkg25Dev.jl.\n",
          "Run Pkg25.add(\"Pkg25Dev\") to install Pkg25Dev on Julia v0.5-")
